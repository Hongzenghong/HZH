...
<body>
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
								<input type="text" class="form-control" name="email"
									id="email_update_input" placeholder="email@hzh.com"> <span
				...
				<div class="modal-body">
					<form class="form-horizontal">
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
								<input type="text" class="form-control" name="email"
									id="email_add_input" placeholder="email@hzh.com"> <span
									class="help-block"></span>
							</div>
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							<label class="col-sm-2 control-label">DeptName</label>
							<div class="col-sm-4">
								<select class="form-control" name="dId" id="dept_app_select">
	...
	<script type="text/javascript">
					...
					.each(
							...
							function(index, item) {
								...
								var editBtn = $("<button></buttn>")
										...
										.append("编辑");
								//为编辑按钮添加一个自定义的属性，来表达当前员工id
								editBtn.attr("edit-id", item.empId);

								var delBtn = $("<button></buttn>")
										.addClass(
		...
		$("#emp_add_modal_btn").click(function() {
			...
			//发送jaxa请求，查出部门信息，显示在下拉列表中
			getDepts("#dept_app_select");
			$("#empAddModal").modal({
				backrop : "static"
			});
		});
		...
		function getDepts(ele) {
			...
			$.ajax({
				...
				success : function(result) {
					// console.log(result)
					//显示部门信息在下拉列表
					//alert(result.extend.depts[0].deptId)
		...
		//显示错误信息
		//抽取去检验信息 ele=位置  status=状态  mgs错误信息
		...
		//1,我们是创建按钮之前就绑定了click，所已绑定不上
		//1可以在创建按钮时候绑定，2绑定单击.live
		...
		function getEmp(id) {
			$.ajax({
				...
				success : function(result) {
					...
					$("#empUpdateModal input[name=genter]").val(
							[ empData.gender ]);
